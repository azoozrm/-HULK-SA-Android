...
sealed class XtreamException(message: String, cause: Throwable? = null) : Exception(message, cause) {
    data object InvalidCredentials : XtreamException("بيانات الاشتراك غير صحيحة أو غير فعالة.")
    data object SubscriptionInactive : XtreamException("الاشتراك منتهي أو غير فعال. تواصل مع الدعم للتجديد.")
    data object InvalidResponse : XtreamException("وصل رد غير صالح من الخدمة.")
    data object ServiceBlocked : XtreamException("الخدمة رفضت الاتصال مؤقتًا. جرّب من شبكة أخرى أو تواصل مع الدعم.")
    data class Http(val statusCode: Int) : XtreamException("تعذر الاتصال بالخدمة (رمز $statusCode).")
    data class Network(val error: IOException) : XtreamException("تحقق من اتصال الإنترنت ثم حاول مرة أخرى.", error)
}
