...
# Retry only a zero-test infrastructure failure. A real assertion or product
# failure must remain final and must never be hidden by retry.
if ! grep -Rqs 'No test results' app/build/outputs/androidTest-results 2>/dev/null; then
  exit "$first_status"
fi
if grep -RqsE 'tests="[1-9][0-9]*"' app/build/outputs/androidTest-results 2>/dev/null; then
  exit "$first_status"
fi
