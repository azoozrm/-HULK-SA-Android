...
def _finding(raw: dict[str, Any], device: str, build_sha: str) -> dict[str, Any]:
    ...
    return {
        ...
        "root_cause_id": raw.get("root_cause_id") or raw.get("root_cause"),
        "finding_role": raw.get("finding_role", "primary"),
        "classification": raw.get("classification") or ("infrastructure" if finding_type == "Infrastructure" else "product"),
        "product_strict": raw.get("product_strict", finding_type == "Product"),
        "gate_outcome": raw.get("gate_outcome"),
        "reclassified_from": raw.get("reclassified_from"),
...
def _markdown(summary: dict[str, Any], findings: list[dict[str, Any]]) -> str:
    lines = [
        ...
        f"- Raw failed checks: {summary.get('raw_failed_checks_count', 0)}",
        f"- Downstream: {summary.get('downstream_count', 0)}",
        f"- Product P0/P1: {summary['product_critical']}",
        f"- Quality Lab P0/P1: {summary.get('quality_lab_critical', 0)}",
        f"- Fixture P0/P1: {summary.get('fixture_critical', 0)}",
...
def aggregate(
    ...
    *,
    ...
    summary = {
        ...
        "warnings": warnings,
        "primary_root_cause_count": len({str(item.get("root_cause_id") or item["code"]) for item in findings if item.get("finding_role", "primary") == "primary" and item["severity"] in {"P0", "P1"}}),
        "raw_failed_checks_count": sum(item["severity"] in {"P0", "P1"} for item in findings),
        "downstream_count": sum(item.get("finding_role") in {"downstream", "blocked_assertion"} for item in findings),
        "quality_lab_critical": sum(item["severity"] in {"P0", "P1"} and item.get("classification") == "quality_lab" for item in findings),
        "fixture_critical": sum(item["severity"] in {"P0", "P1"} and item.get("classification") == "fixture" for item in findings),
        "false_positives": sum(item["finding_type"] == "False positive" or bool(item.get("reclassified_from")) for item in findings),
        "devices": devices,
