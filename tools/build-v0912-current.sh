...
python3 tools/apply-v09114.py project
grep -q 'versionName = "0.9.1.14"' project/app/build.gradle.kts
python3 tools/apply-v09119.py project
grep -q 'versionName = "0.9.1.19"' project/app/build.gradle.kts
...
PY
cd project
gradle --no-daemon :app:testDebugUnitTest :app:assembleDebug -PHULK_PORTAL_URL=http://3162356.xyz:8080 --stacktrace 2>&1 | tee ../v09119-build.log
cd ..
cp project/app/build/outputs/apk/debug/app-debug.apk output/HULK-SA-v0.9.1.19-current-beta.apk
sha256sum output/HULK-SA-v0.9.1.19-current-beta.apk | tee output/SHA256.txt
