...
def validate_repo(repo_root: Path, expected_logo_sha256: str = DEFAULT_LOGO_SHA256) -> list[Check]:
    ...
    logo_file = repo_root / "app/src/main/res/drawable-nodpi/hulk_sa_logo.webp"
    if logo_file.is_file():
        actual_logo_sha = sha256(logo_file)
        add("approved-logo-sha256", actual_logo_sha == expected_logo_sha256,
            "Approved HULK SA logo bytes are unchanged",
            f"Approved logo hash changed: {actual_logo_sha}", [str(logo_file)])
    else:
        checks.append(Check("approved-logo-present", "FAIL", "Approved HULK SA logo file is missing", [str(logo_file)]))
    ...
    add("legacy-lab-removed", not present_legacy,
        ...
        "Legacy paths remain: " + ", ".join(present_legacy), present_legacy or ["repository tree"])
...
def write_reports(checks: list[Check], out_dir: Path) -> dict[str, object]:
    ...
    for check in checks:
        if check.status == "FAIL":
            body = f'<failure message={json.dumps(check.message)} />'
        elif check.status in ("BLOCKED", "SKIPPED"):
            body = f'<skipped message={json.dumps(check.message)} />'
        else:
            body = ""
        cases.append(f'<testcase classname="compatibility.v2.static" name="{check.id}">{body}</testcase>')
    junit = (
        f'<testsuite name="compatibility-v2-static" tests="{len(checks)}" failures="{failures}" skipped="{skipped}">' +
